...
from data.preprocess import MyDataset, MembraneDataLoader, TimeSeriesPreprocessor
from data.normalize import Normalizer_ts
from train.train import test_rollout, train_regular_transformer
...
output_dir = 'train/train_output/LSTMOG'
...
for epoch in tqdm(range(epochs)):
    ...
    temp_error = train_regular_transformer(model, device, train_loader, optimizer, criterion,
                                                       scheduler=scheduler,
    ...
    if (epoch % save_interval == 0) and True:
        ...
        # rollout
        pred, gt = test_rollout(model, device, processed_train_inputs, seq_len=seq_len, x_normalizer=preprocessor.normalizer)
