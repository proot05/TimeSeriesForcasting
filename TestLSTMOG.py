...
from testfuncs.predict import predict_og
...
input_dir = 'train/train_output/LSTMOG'
output_dir = 'testfuncs/test_output/LSTMOG'

if ose(output_dir):
    shutil.rmtree(output_dir)
os.mkdir(output_dir)

state = TimeSeriesPreprocessor.load_state(input_dir + '/checkpoints' + '/LSTMOG_normalizer.pkl')
new_dt = state['dt_new']
...
for i in tqdm(range(int((len(mem_time_test) - seq_len)))):
    ...
    pred, dt = predict_og(model, mem_time_test[i:i + seq_len], mem_data_test[i:i + seq_len], new_dt, rnn_delay,
