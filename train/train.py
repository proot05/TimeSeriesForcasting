...
#             # output, hidden = model(temp_inputs, hidden)  # Assuming model outputs the prediction for the current time step
#             output = model(temp_inputs)
#
...
#         # output, hidden = model(
#         #     data[i:i+seq_len].unsqueeze(0).to(device),
#         #     hidden
#         # )  # <B,L,C>
#         output = model(data[:i + seq_len].unsqueeze(0).to(device),)  # <B,L,C>
